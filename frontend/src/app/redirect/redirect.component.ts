...
export class RedirectComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      const hasPassword = await fetch(
        `http://localhost:4200/api/checkIfPassord?short_link=${this.shortLink}`,
        {
          method: 'GET',
          credentials: 'include',
  ...
  async redirect() {
    const url = new URL('http://localhost:4200/api/getUrl?short_link=salut_');
    ...
    try {
      ...
      try {
        await fetch('http://localhost:4200/api/addClick', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
